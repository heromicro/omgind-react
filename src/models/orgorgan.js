  ...
  effects: {
    *fetch({ search, pagination }, { call, put, select }) {
      ...
      if (search) {
        ...
        const s = yield select((state) => state.orgorgan.search);
        if (s) {
          params = { ...params, ...s };
        }
      }
      ...
      if (pagination) {
        ...
        const p = yield select((state) => state.orgorgan.pagination);
        if (p) {
          params = { ...params, ...p };
        }
      }

      const response = yield call(organService.query, params);
    ...
    *fetchForm({ payload }, { call, put }) {
      const response = yield call(organService.get, payload.id);
      const { code, burden } = response;
      if (code === 0) {
        yield [
          put({
    ...
    *loadDetail({ payload }, { call, put, select }) {
      ...
      const response = yield call(organService.view, record.id);
    ...
    *submit({ payload, callback = null }, { call, put, select }) {
      ...
      const formType = yield select((state) => state.orgorgan.formType);
      let success = false;
      if (formType === 'E') {
        const id = yield select((state) => state.orgorgan.formID);
        const response = yield call(organService.update, id, params);
        const { code } = response;
        if (code === 0) {
          success = true;
        }
      } else {
        const response = yield call(organService.create, params);
        const { code, burden } = response;
        if (code === 0) {
          success = true;
        }
    ...
    *del({ payload }, { call, put }) {
      const response = yield call(organService.del, payload.id);
      const { code } = response;
      if (code === 0) {
    ...
    *changeStatus({ payload }, { call, put, select }) {
      ...
      if (payload.is_active === true) {
        response = yield call(organService.enable, payload.id);
      } else {
        response = yield call(organService.disable, payload.id);
      }
      ...
      if (code === 0) {
        ...
        message.success(msg);
        const data = yield select((state) => state.orgorgan.data);

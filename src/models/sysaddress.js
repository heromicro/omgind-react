  ...
  effects: {
    *fetch({ search, pagination }, { call, put, select }) {
      ...
      if (search) {
        ...
        const s = yield select((state) => state.sysaddress.search);
        if (s) {
          params = { ...params, ...s };
        }
      }
      ...
      if (pagination) {
        ...
        const p = yield select((state) => state.sysaddress.pagination);
        if (p) {
          params = { ...params, ...p };
        }
      }
    ...
    *submit({ payload, callback = null }, { call, put, select }) {
      ...
      const formType = yield select((state) => state.sysaddress.formType);
      let success = false;
      if (formType === 'E') {
    ...
    *changeStatus({ payload }, { call, put, select }) {
      ...
      if (code === 0) {
        ...
        message.success(msg);
        const data = yield select((state) => state.sysaddress.data);
